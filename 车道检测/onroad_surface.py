...
class LaneDetector:
    ...
    def processSingleImage(self,img):
        ...
        self.moveup=moveup2/100
        self.wider = wider / 100

        undist=self.undistort(img)
        self.roi_corners=[[(0.16-self.wider)*undist.shape[1],(1-self.moveup)*undist.shape[0]],
                    [(0.45-self.wider)*undist.shape[1],(0.63-self.moveup)*undist.shape[0]],
                    [(0.55+self.wider)*undist.shape[1],(0.63-self.moveup)*undist.shape[0]],
                    [(0.84+self.wider)*undist.shape[1],(1-self.moveup)*undist.shape[0]]]
...
# out = cv2.VideoWriter('output.avi',fourcc, 20.0, (imgc.shape[1], imgc.shape[0]))

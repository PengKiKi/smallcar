...
class LaneDetector:
    ...
    def unwarp(self, undist):
        ...
        offset = flag3
        warped_size = (orgw + 2 * offset, undist.shape[1])
        dst = np.float32([
    ...
    def processSingleImage(self, img):
        ...
        undist = self.undistort(img)
        self.roi_corners = [[0.16 * undist.shape[1], (1 - self.moveup) * undist.shape[0]],
                            [0.45 * undist.shape[1], (0.63 - self.moveup) * undist.shape[0]],
                            [0.55 * undist.shape[1], (0.63 - self.moveup) * undist.shape[0]],
                            [0.84 * undist.shape[1], (1 - self.moveup) * undist.shape[0]]]
...
while True:
    ...
    img=img[0:int(img.shape[0] * (1-moveup2/100)), :, :]
